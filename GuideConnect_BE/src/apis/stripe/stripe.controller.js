// import { Payment, Bill, BillPayment } from '../models/stripeModel';
// import stripePackage from 'stripe';
import mongoose from 'mongoose';
import PaymentDemoModel from '../../models/paymentDemo.model'
...
class StripeController {
    ...
    //                 user_id: mongoose.Types.ObjectId(session.metadata.user_id),
    ...
    //                     bill_id: mongoose.Types.ObjectId(item.price.product.metadata.bill_id),
    ...
    async Payments(req, res) {
        try {
            const { user_id, guide_id, price, content } = req.body;
            const newPayment = new PaymentDemoModel({
                user_id,
                guide_id,
                price,
                content
            });
            const bill = await newPayment.save();
            res.status(200).json({ message: 'Payment success handled', bill });
        } catch (error) {
            console.error('Error handling payment success:', error);
            res.status(500).json({ error: 'An error occurred while handling payment success' });
